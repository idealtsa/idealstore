...
const getAllUsers = async (req, res) => {
  try {
    ...
    const ActiveUser=await User.count({status:'Active'});
    const SellerUser=await User.count({role:'seller'});
    const AdminUser=await User.count({role:'admin'});
    res.status(200).json({
      TotalUser,
      ActiveUser,
      AdminUser,
...
//Post Method for updateuser
const updateuser = async (req, res, next) => {
  try {
    // Get user input
    const _id = req.params.id;
    let { name, cellno, password, passwordConfirmation, email } = req.body;
    // Validate user input
    if (!(password && name && cellno && passwordConfirmation && email)) {
      return res.status(400).json({ message: "All input is required" });
    }
    ...
    let filter = { _id: _id };
    let update = { name, cellno, encryptedPassword };
